...
from src.mcqr.mcqr_regression import Supervised_MC_QR_prediction_for_regression
...
class Model(LightningModule):
    ...
    def __init__(self, hparams):
        ...
        self.non_mp_loss = loss.NonMPLoss()
        self.mcqr_loss = loss.MCQRLossHeat()
    ...
    def _build_model(self):
        ...
        self.model = getattr(models, layout_model)(in_channels=2) ##########################################
    ...
    def prepare_data(self):
        ...
        # split train/val set
        train_length, val_length = int(len(trainval_dataset) * 0.9), int(len(trainval_dataset) * 0.1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        ax1 = heat_obs.size(0)
        taus = torch.rand(ax1, 1, 1, 1).to(device=heat_obs.device)
        ones = torch.ones_like(heat_obs).to(device=heat_obs.device)
        zeros = torch.zeros_like(heat_obs).to(device=heat_obs.device)
        taus_mat = torch.where(heat_obs==0, zeros, ones) * taus
        heat_obs_taus = torch.cat((heat_obs, taus_mat), dim=1)

        # taus = torch.rand(ax1, 1, 1, 1).to(device=heat_obs.device)
        # taus_mat = (torch.ones_like(heat_obs)* taus).to(device=heat_obs.device)
        # heat_obs_taus = torch.cat((heat_obs, taus_mat), dim=1)

        heat_pred = self(heat_obs_taus)
        
        loss_mcqr = self.pointloss(heat_obs, heat_pred, taus_mat) * 1e5
        loss_non_mp = self.non_mp_loss(heat_obs, heat, heat_pred) * 1e3
        loss_tv = self.tvloss(heat_pred) * 1e2

        loss = loss_mcqr + loss_non_mp + loss_tv
        self.log("train/training_mae", loss_non_mp * self.hparams.std_heat)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        ones = torch.ones_like(heat_obs).to(device=heat_obs.device)
        zeros = torch.zeros_like(heat_obs).to(device=heat_obs.device)
        taus_mat = torch.where(heat_obs==0, zeros, ones) * 0.5
        heat_obs_taus = torch.cat((heat_obs, taus_mat), dim=1)
        heat_pred = self(heat_obs_taus)
    ...
    def test_step(self, batch, batch_idx):
        ...
        # heat_pred = self(heat_obs_taus)
        with torch.no_grad():
            heat_pred, _= Supervised_MC_QR_prediction_for_regression(100, heat_obs, self, tau='all')

        loss_mae = self.criterion(heat_pred, heat) * self.hparams.std_heat
        return {"mae": loss_mae}
    ...
    #     ones = torch.ones_like(default_layout).to(device=heat_obs_taus.device)
    #     zeros = torch.zeros_like(default_layout).to(device=heat_obs_taus.device)
    ...
    #     #---------------------------------
    #     boundary_ones = torch.zeros_like(default_layout).to(device=heat_obs_taus.device)
    ...
    def add_model_specific_args(parser):  # pragma: no-cover
        ...
        parser.add_argument("--rev_model", type=bool, default=False, help="Reversible Regression Model or not")
