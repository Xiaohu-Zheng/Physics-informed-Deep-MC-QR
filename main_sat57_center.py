...
from src import mcs_pre, train, test, plot, predict, interval_prob, data_noise, mcs_pre, train_supervised, plot_supervised
...
def main():
    ...
    config_path = Path(__file__).absolute().parent / "config/config_sat57_center.yml"
    data_path = Path(__file__).absolute().parent / "config/data_sat57_center.yml"
    ...
    # running
    assert hparams.mode in ["train", "test", "plot", "predict", "interval_prob", 
                            "data_noise", "mcs_pre", "train_supervised", "plot_supervised"]
