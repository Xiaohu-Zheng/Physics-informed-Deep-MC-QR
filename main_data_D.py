...
from src import train, test, plot, predict, interval_prob, data_noise
...
def main():
    ...
    config_path = Path(__file__).absolute().parent / "config/configD.yml"
    data_path = Path(__file__).absolute().parent / "config/dataD.yml"
    ...
    # running
    assert hparams.mode in ["train", "test", "plot", "predict", "interval_prob", "data_noise"]
